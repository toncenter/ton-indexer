...
class TonlibClientSync:
    ...
    def raw_getBlockTransactions(self, fullblock, count, after_tx):
        return self.thread.run_coro(self.client_impl.raw_getBlockTransactions(fullblock, count, after_tx))

    def raw_getBlockTransactionsExt(self, fullblock, count, after_tx):
        return self.thread.run_coro(self.client_impl.raw_getBlockTransactionsExt(fullblock, count, after_tx))

    def getTransactions(self, account_address, from_transaction_lt=None, from_transaction_hash=None, to_transaction_lt=0, limit=10):
    ...
    def lookupBlock(self, workchain, shard, seqno=None, lt=None, unixtime=None):
        return self.thread.run_coro(self.client_impl.lookupBlock(workchain, shard, seqno, lt, unixtime))

    def getShards(self, master_seqno=None, lt=None, unixtime=None):
        return self.thread.run_coro(self.client_impl.getShards(master_seqno, lt, unixtime))

    def getBlockTransactions(self, workchain, shard, seqno, count, root_hash=None, file_hash=None, after_lt=None, after_hash=None):
    ...
    def getBlockTransactionsExt(self, workchain, shard, seqno, count, root_hash=None, file_hash=None, after_lt=None, after_hash=None):
        return self.thread.run_coro(self.client_impl.getBlockTransactionsExt(workchain, shard, seqno, count, root_hash, file_hash, after_lt, after_hash))

    def getBlockHeader(self, workchain, shard, seqno, root_hash=None, file_hash=None):
        return self.thread.run_coro(self.client_impl.getBlockHeader(workchain, shard, seqno, root_hash, file_hash))

    def tryLocateTxByIncomingMessage(self, source, destination, creation_lt):
        return self.thread.run_coro(self.client_impl.tryLocateTxByIncomingMessage(source, destination, creation_lt))

    def tryLocateTxByOutcomingMessage(self, source, destination, creation_lt):
