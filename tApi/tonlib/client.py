...
from tApi.tonlib.utils import TonLibWrongResult, b64str_to_hex, hex_to_b64str, hash_to_hex
...
class TonlibClient:
    ...
    async def tryLocateTxByIncomingMessage(self, source, destination, creation_lt):
        ...
        for shard_data in shards['shards']:
            ...
            for b in range(3):
                ...
                txs = await self.getBlockTransactions(workchain, shardchain, block["seqno"], count=40, root_hash=block["root_hash"], file_hash=block["file_hash"])
                candidate = tuple()
                count = 0
                ...
                if candidate:
                    txses = await self.getTransactions(destination, from_transaction_lt=candidate[1], from_transaction_hash=b64str_to_hex(candidate[0]), limit=max(count, 10))
                    for tx in txses:
                        try:
                            in_msg = tx["in_msg"]
    ...
    async def tryLocateTxByOutcomingMessage(self, source, destination, creation_lt):
        ...
        for shard_data in shards['shards']:
            ...
            txses = await self.getBlockTransactions(workchain, shardchain, block["seqno"], count=40, root_hash=block["root_hash"], file_hash=block["file_hash"])
            candidate = tuple()
            count = 0
            for tx in txses["transactions"]:
                if tx["account"] == src["raw_form"]:
                    ...
                    if not candidate or candidate[1] < int(tx["lt"]):
                        candidate = tx["hash"], int(tx["lt"])
            if candidate:
                txses = await self.getTransactions(source, from_transaction_lt=candidate[1], from_transaction_hash=b64str_to_hex(candidate[0]), limit=max(count, 10))
                for tx in txses:
                    try:
