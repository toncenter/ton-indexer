...
module through `ton-index-go/go.mod` and checks in its pinned catalog and tests.
Go bindings are ignored build artifacts generated by CMake, Docker, and CI using
that pinned Acton module. For direct Go builds or tests, first run
`CGO_ENABLED=0 go generate ./index/acton/catalog` from `ton-index-go`.
This generation path needs no installed Acton Rust CLI; the build guide also
documents `acton wrapper --catalog ... --go` for Acton builds with Go support.
