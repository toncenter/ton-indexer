...
func jettonTransfersQueryParts(req models.JettonTransferRequest, sortOrder string) txLtListParts {
	...
	clmn_query := `T.tx_hash, T.tx_lt, T.tx_now, T.tx_aborted, T.query_id,
		T.amount, coalesce(T.source, '') as source, coalesce(T.destination, '') as destination, coalesce(T.jetton_wallet_address, '') as jetton_wallet_address, coalesce(T.jetton_master_address, '') as jetton_master_address, T.response_destination, T.custom_payload,
...
func jettonBurnsQueryParts(req models.JettonBurnRequest, sortOrder string) txLtListParts {
	...
	clmn_query := `T.tx_hash, T.tx_lt, T.tx_now, T.tx_aborted, T.query_id,
		coalesce(T.owner, '') as owner, coalesce(T.jetton_wallet_address, '') as jetton_wallet_address, coalesce(T.jetton_master_address, '') as jetton_master_address, T.amount, T.response_destination, T.custom_payload, T.trace_id`
...
func (db *DbClient) QueryJettonMasters(
	...
	if db.Kvrocks != nil {
		ctx, cancel_ctx := context.WithTimeout(context.Background(), settings.Timeout)
		defer cancel_ctx()
		res, err := db.Kvrocks.QueryJettonMasters(ctx, req, settings)
		if err != nil {
...
func (db *DbClient) QueryJettonWallets(
	...
	if db.Kvrocks != nil {
		ctx, cancel_ctx := context.WithTimeout(context.Background(), settings.Timeout)
		defer cancel_ctx()
		res, err := db.Kvrocks.QueryJettonWallets(ctx, req, settings)
		if err != nil {
			return nil, nil, nil, models.IndexError{Code: 500, Message: err.Error()}
		}
		addr_list := []models.AccountAddress{}
