...
func (db *DbClient) QueryPendingActions(settings models.RequestSettings, emulatedContext *EmulatedTracesContext, request models.PendingActionsRequest) ([]models.Action, models.AddressBook, models.Metadata, error) {
	...
	if request.IncludeTransactions != nil && *request.IncludeTransactions {
		...
		if err != nil {
			return nil, nil, nil, models.IndexError{Code: 500, Message: fmt.Sprintf("failed query transactions: %s", err.Error())}
		}
		txMap := make(map[models.HashType]*models.Transaction)
		for idx := range txs {
			tx := &txs[idx]
			txMap[tx.Hash] = tx
		}
		...
		for i := range actions {
			actions[i].Transactions = make([]*models.Transaction, 0, len(actions[i].TxHashes))
...
func QueryPendingTransactionsImpl(emulatedContext *EmulatedTracesContext, conn *pgxpool.Conn, settings models.RequestSettings, filterCompletedTransaction bool) ([]models.Transaction, error) {
...
func queryPendingActionsImpl(emulatedContext *EmulatedTracesContext, conn *pgxpool.Conn, settings models.RequestSettings, request models.PendingActionsRequest) ([]models.RawAction, error) {
	completed_traces, err := queryCompletedEmulatedTraces(emulatedContext, conn, settings, true)
	if err != nil {
		return nil, models.IndexError{Code: 500, Message: err.Error()}
	}
