...
func parseJettonTransferBodyTlb(slice *cell.Slice) (*JettonTransferBody, error) {
	...
	if tlbStruct.ForwardPayload != nil {
		...
		parseForwardPayload(jt, tlbStruct.ForwardPayload.BeginParse())
	}

	return jt, nil
}

func parseForwardPayload(jt *JettonTransferBody, payloadSlice *cell.Slice) {
	...
	case 0x25938561:
		...
		parseStonfiSwap(jt, payloadSlice)
	default:
		jt.PayloadSumType = "Unknown"
	}
}

func parseStonfiSwap(jt *JettonTransferBody, slice *cell.Slice) {
	defer func() {
		if recover() != nil {
			jt.PayloadSumType = "Unknown"
		}
