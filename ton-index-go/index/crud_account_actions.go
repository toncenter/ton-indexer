...
func roleInClause(role *RoleType) string {
	roleIds := roleToIds(role)
	parts := make([]string, len(roleIds))
	for i, id := range roleIds {
		parts[i] = strconv.Itoa(id)
...
func (db *DbClient) QueryAccountActions(
...
) ([]Action, AddressBook, Metadata, string, error) {
	...
	if lim_req.Limit != nil {
		...
		if limit > settings.MaxLimit {
			return nil, nil, nil, "", IndexError{Code: 422,
	...
	if req.Cursor != nil {
		...
		if err != nil {
			return nil, nil, nil, "", IndexError{Code: 422, Message: "invalid cursor"}
		}
	}

	// Collect actions grouped by trace until we have `limit` traces.
	traceActions := make(map[string][]RawAction)
	traceOrder := []string{}
	batchSize := limit * 5
	maxIterations := 10
	...
	for _, tid := range traceOrder {
		for idx := range traceActions[tid] {
			CollectAddressesFromAction(&addr_map, &traceActions[tid][idx])
			action, err := ParseRawAction(&traceActions[tid][idx])
			if err != nil {
				return nil, nil, nil, "", IndexError{Code: 500, Message: err.Error()}
			}
...
func queryRawActionsCtx(ctx context.Context, query string, args []any, conn *pgxpool.Conn) ([]RawAction, error) {
