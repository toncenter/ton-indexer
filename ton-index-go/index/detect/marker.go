...
func MarkMessagesByPtr(messages []*models.Message) error {
	refs := &messagesRefs{
...
func MarkMessages(messages []models.Message) error {
	refs := collectMessagesRefs(messages)
	return markWithRefs(refs)
}

func MarkJettonTransfers(transfers []models.JettonTransfer) error {
	refs := collectJettonTransfersRefs(transfers)
	return markWithRefs(refs)
}

func MarkJettonBurns(burns []models.JettonBurn) error {
	refs := collectJettonBurnsRefs(burns)
	return markWithRefs(refs)
}

func MarkNFTTransfers(transfers []models.NFTTransfer) error {
	refs := collectNFTTransfersRefs(transfers)
	return markWithRefs(refs)
}

func collectMessagesRefs(messages []models.Message) *messagesRefs {
	refs := &messagesRefs{
		opcodeRefs: make(map[uint32][]*(*string)),
		bodyRefs:   make(map[string][]*(*json.RawMessage)),
	}
...
func collectJettonTransfersRefs(transfers []models.JettonTransfer) *messagesRefs {
	refs := &messagesRefs{
		opcodeRefs: make(map[uint32][]*(*string)),
		bodyRefs:   make(map[string][]*(*json.RawMessage)),
	}
...
func collectJettonBurnsRefs(burns []models.JettonBurn) *messagesRefs {
	refs := &messagesRefs{
		opcodeRefs: make(map[uint32][]*(*string)),
		bodyRefs:   make(map[string][]*(*json.RawMessage)),
	}
...
func collectNFTTransfersRefs(transfers []models.NFTTransfer) *messagesRefs {
	refs := &messagesRefs{
		opcodeRefs: make(map[uint32][]*(*string)),
		bodyRefs:   make(map[string][]*(*json.RawMessage)),
	}
...
func collectSingleMessageRefs(msg *models.Message, refs *messagesRefs) {
	if msg == nil {
		return
	}
	// collect opcodes
