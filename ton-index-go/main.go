...
func GetAccountActions(c *fiber.Ctx) error {
	...
	if len(req.AccountAddress) == 0 {
		return models.IndexError{Code: 422, Message: "account is required"}
	}

	if req.Role != nil && *req.Role != models.RoleSender && *req.Role != models.RoleReceiver &&
		*req.Role != models.RoleInitiated && *req.Role != models.RoleObserver {
		return models.IndexError{Code: 422, Message: "role must be 'sender', 'receiver', 'initiated', or 'observer'"}
	}
	...
	resp := models.AccountActionsResponse{
		Actions:     actions,
		AddressBook: book,
		Metadata:    metadata,
		Cursor:      cursor,
...
func main() {
	...
	parse.IsTestnet = settings.Request.IsTestnet
