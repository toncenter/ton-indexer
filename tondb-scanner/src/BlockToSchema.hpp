...
public:
  ...
  td::Status parse() {
    if (!is_parsed_) {
      TRY_STATUS(parse_impl());
      is_parsed_ = true;
    }
    return td::Status::OK();
  }

  const std::vector<schema::Block> get_blocks() const {
    return blocks_;
  }

  const std::vector<schema::Transaction> get_transactions() const {
    return transactions_;
  }

  const std::vector<schema::Message> get_messages() const {
    return messages_;
  }

  const std::vector<schema::TransactionMessage> get_transaction_messages() const {
    return transaction_messages_;
  }

  const std::vector<schema::AccountState> get_account_states() const {
    return account_states_;
  }
...
private:
  td::Status parse_impl() {
    ...
    for (auto &block_ds : mc_block_) {
      ...
      blocks_.push_back(schema_block);
  ...
  td::Status parse_account_states(const td::Ref<ShardState>& block_state, std::set<td::Bits256> &addresses) {
    ...
    for (auto &addr : addresses) {
      ...
      case block::gen::Account::account: {
        ...
        account_states_.push_back(account);
        break;
      }
      default:
    ...
    LOG(DEBUG) << "Parsed " << account_states_.size() << " account states";
    return td::Status::OK();
  }
