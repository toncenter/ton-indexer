...
ShardStateDataPtr build_shard_state_data() {
}


ShardStateScanner::ShardStateScanner(td::Ref<vm::Cell> shard_state, MasterchainBlockDataState mc_block_ds,
                                     Options options)
...
void ShardStateScanner::start_up() {
    ...
    td::actor::send_closure(actor_id(this), &ShardStateScanner::schedule_next);
}
...
void ShardStateScanner::batch_parsed(td::Bits256 last_addr, std::vector<InsertData> results) {
    ...
    auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), cnt = results.size(), last_addr](td::Result<td::Unit> R) {
        if (R.is_error()) {
