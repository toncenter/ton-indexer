...
bool is_gasless_request_opcode(std::uint32_t opcode) {
  return opcode == kTgWalletSendOneMessageInternal || opcode == kTgWalletSendBulkMessagesInternal ||
...
std::optional<std::uint32_t> get_tg_wallet_request_opcode(const Message *message) {
  ...
  if (r_raw.is_error()) return std::nullopt;
  ...
  try {
    ...
    return is_tg_wallet_request_opcode(opcode) ? std::optional<std::uint32_t>(opcode) : std::nullopt;
  } catch (...) {
    return std::nullopt;
  }
}
