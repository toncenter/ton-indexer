...
// class methods
void BlockInterfaceProcessor::start_up() {
  timer_.resume();
  std::unordered_map<block::StdAddress, schema::AccountState>
      account_states_to_detect;
  for (const auto &account_state : block_->account_states_) {
    ...
    if (existing == account_states_to_detect.end() ||
        account_state.last_trans_lt > existing->second.last_trans_lt) {
  ...
  auto P = td::PromiseCreator::lambda(
      [&, SelfId = actor_id(this)](td::Result<td::Unit> res) mutable {
        td::actor::send_closure(SelfId, &BlockInterfaceProcessor::finish,
                                std::move(res));
      });
  ...
  for (const auto &[_, account_state] : account_states_to_detect) {
    ...
    td::actor::create_actor<FullDetector>(
        ...
        td::PromiseCreator::lambda(
            [SelfId = actor_id(this), account_state,
             promise = ig.get_promise()](
                std::vector<typename FullDetector::DetectedInterface>
                    interfaces) mutable {
              td::actor::send_closure(
                  SelfId, &BlockInterfaceProcessor::process_address_interfaces,
                  account_state.account, std::move(interfaces),
                  account_state.code_hash.value(),
                  account_state.data_hash.value(), account_state.last_trans_lt,
                  account_state.timestamp, std::move(promise));
            }))
        .release();
  }
}
void BlockInterfaceProcessor::process_address_interfaces(
    block::StdAddress address,
    std::vector<FullDetector::DetectedInterface> interfaces,
    td::Bits256 code_hash, td::Bits256 data_hash, uint64_t last_trans_lt,
    uint32_t last_trans_now, td::Promise<td::Unit> promise) {
    ...
    std::visit(
        [&](auto &&arg) {
          ...
          if constexpr (std::is_same_v<T, JettonMasterDetectorR::Result>) {
            ...
            jetton_master_data.jetton_wallet_code_hash =
                arg.jetton_wallet_code_hash.bits();
            ...
            if (arg.dns_entry) {
              nft_item_data.dns_entry = schema::NFTItemDataV2::DNSEntry{
                  arg.dns_entry->domain, arg.dns_entry->wallet,
                  arg.dns_entry->next_resolver, arg.dns_entry->site_adnl,
            ...
            fix_price_sale_data.marketplace_fee_address =
                arg.marketplace_fee_address;
            ...
            interfaces_[address].push_back(fix_price_sale_data);
                                   ...
                                   T, GetGemsNftFixPriceSaleV4::Result>) {
            ...
            fix_price_sale_v4_data.marketplace_address =
            ...
            fix_price_sale_v4_data.marketplace_fee_address =
                arg.marketplace_fee_address;
...
void BlockInterfaceProcessor::finish(td::Result<td::Unit> status) {
  if (status.is_error()) {
