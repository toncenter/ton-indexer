  ...
  if (create_indexes_) {
    ...
    try {
      ...
      query += (
        ...
        "create index if not exists dns_entries_index_4 on dns_entries (nft_item_owner, dns_wallet, length(domain)) include (domain) where ((nft_item_owner)::text = (dns_wallet)::text);\n"
