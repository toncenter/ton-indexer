...
#define TONADDR_KIND_OFFSET 0
#define TONADDR_WORKCHAIN_OFFSET 1
#define TONADDR_STD_ADDR_OFFSET 5
#define TONADDR_EXT_BITS_OFFSET 1
#define TONADDR_EXT_ADDR_OFFSET 3
#define TONADDR_VAR_WORKCHAIN_OFFSET 1
#define TONADDR_VAR_BITS_OFFSET 5
#define TONADDR_VAR_ADDR_OFFSET 7
...
static bool
tonaddr_var_hex_len_from_text(const char *src, int len, int *bit_len, int *hex_len, bool *has_completion) {
    *has_completion = len > 0 && src[len - 1] == '_';
    *hex_len = *has_completion ? len - 1 : len;
    ...
    if (*has_completion) {
        ...
        last_nibble = tonaddr_hex_value(src[*hex_len - 1]);
        if (last_nibble <= 0) {
            return false;
        }
        while (trailing_zeroes < 4 && (last_nibble & (1 << trailing_zeroes)) == 0) {
            trailing_zeroes++;
        }
        if (trailing_zeroes > 2) {
            return false;
        }
        *bit_len = (*hex_len - 1) * 4 + (3 - trailing_zeroes);
    } else {
        *bit_len = *hex_len * 4;
    }

    if (*bit_len > TON_ADDR_MAX_BITS) {
        return false;
    }
    for (int i = 0; i < *hex_len; ++i) {
        if (tonaddr_hex_value(src[i]) < 0) {
            return false;
        }
    }
    return true;
...
static bool
tonaddr_decode_var_hex(const char *src, int bit_len, int hex_len, bool has_completion, uint8 *dst) {
    if (!tonaddr_decode_hex_nibbles(src, hex_len, dst)) {
        return false;
    }
    if (has_completion) {
        int partial_bits = bit_len & 3;
        int last_index = hex_len - 1;
        int last_nibble = tonaddr_hex_value(src[last_index]) & (0x0f << (4 - partial_bits));

        if ((last_index & 1) == 0) {
            dst[last_index / 2] = (uint8) ((dst[last_index / 2] & 0x0f) | (last_nibble << 4));
        } else {
            dst[last_index / 2] = (uint8) ((dst[last_index / 2] & 0xf0) | last_nibble);
        }
    }
    return true;
}
...
static int
tonaddr_var_text_hex_len(int bit_len) {
    int hex_len = tonaddr_hex_len_from_bits(bit_len);

    return hex_len + ((bit_len & 3) != 0 ? 1 : 0);
}

static void
tonaddr_encode_var_hex(const uint8 *src, int bit_len, char *dst) {
    static const char digits[] = "0123456789ABCDEF";
    int hex_len = tonaddr_hex_len_from_bits(bit_len);
    int partial_bits = bit_len & 3;

    tonaddr_encode_hex_nibbles(src, hex_len, dst);
    if (partial_bits != 0) {
        int last_index = hex_len - 1;
        int last_nibble = tonaddr_hex_value(dst[last_index]);

        last_nibble = (last_nibble & (0x0f << (4 - partial_bits))) | (1 << (3 - partial_bits));
        dst[last_index] = digits[last_nibble];
        dst[hex_len] = '_';
    }
}
...
tonaddr_make_none(void) {
    TonAddr *result = tonaddr_alloc(1);
    uint8 *data = (uint8*) VARDATA(result);

    data[TONADDR_KIND_OFFSET] = TONADDR_KIND_NONE;
    return result;
}
...
tonaddr_validate_payload(const uint8 *data, int len) {
    ...
    switch (data[TONADDR_KIND_OFFSET]) {
        ...
        case TONADDR_KIND_EXT:
            ...
            bit_len = tonaddr_read_u16(data + TONADDR_EXT_BITS_OFFSET);
        ...
        case TONADDR_KIND_VAR:
            ...
            bit_len = tonaddr_read_u16(data + TONADDR_VAR_BITS_OFFSET);
            if (bit_len < 0 || bit_len > TON_ADDR_MAX_BITS) {
                return false;
            }
...
Datum tonaddr_in(PG_FUNCTION_ARGS) {
    ...
    if (strncmp(str, "ext$", 4) == 0) {
        ...
        data[TONADDR_KIND_OFFSET] = TONADDR_KIND_EXT;
        tonaddr_write_u16(data + TONADDR_EXT_BITS_OFFSET, (int) bit_len);
    ...
    if (strncmp(str, "var$", 4) == 0) {
        ...
        addr_start++;
        if (!tonaddr_var_hex_len_from_text(addr_start, len - (addr_start - str),
                                           &bit_len, &hex_len, &has_completion)) {
            ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("invalid variable address hex value for type %s: \"%s\"", "tonaddr", str)));
        }

        payload_len = TONADDR_VAR_ADDR_OFFSET + tonaddr_data_len_from_bits(bit_len);
        result = tonaddr_alloc(payload_len);
        data = (uint8*) VARDATA(result);
        data[TONADDR_KIND_OFFSET] = TONADDR_KIND_VAR;
        tonaddr_write_i32(data + TONADDR_VAR_WORKCHAIN_OFFSET, (int32) workchain);
        tonaddr_write_u16(data + TONADDR_VAR_BITS_OFFSET, bit_len);
        if (!tonaddr_decode_var_hex(addr_start, bit_len, hex_len, has_completion, data + TONADDR_VAR_ADDR_OFFSET)) {
            pfree(result);
            ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("failed to decode hex value for type %s", "tonaddr")));
        }
    ...
    uint8 *data = (uint8*) VARDATA(result);
    data[TONADDR_KIND_OFFSET] = TONADDR_KIND_STD;
    tonaddr_write_i32(data + TONADDR_WORKCHAIN_OFFSET, (int32) workchain);
...
Datum tonaddr_out(PG_FUNCTION_ARGS) {
    ...
    if (data[TONADDR_KIND_OFFSET] == TONADDR_KIND_NONE) {
        result = psprintf("addr_none");
        PG_FREE_IF_COPY(addr, 0);
        PG_RETURN_CSTRING(result);
    }

    if (data[TONADDR_KIND_OFFSET] == TONADDR_KIND_EXT) {
        int bit_len = tonaddr_read_u16(data + TONADDR_EXT_BITS_OFFSET);
    ...
    if (data[TONADDR_KIND_OFFSET] == TONADDR_KIND_VAR) {
        int32 workchain_id = tonaddr_read_i32(data + TONADDR_VAR_WORKCHAIN_OFFSET);
        int bit_len = tonaddr_read_u16(data + TONADDR_VAR_BITS_OFFSET);

        workchain_len = snprintf(workchain, sizeof(workchain), "%d", workchain_id);
        hex_len = tonaddr_var_text_hex_len(bit_len);
        full_len = 4 + workchain_len + 1 + hex_len;
        result = (char*) palloc(full_len + 1);
        memcpy(result, "var$", 4);
        memcpy(result + 4, workchain, workchain_len);
        result[4 + workchain_len] = ':';
        tonaddr_encode_var_hex(data + TONADDR_VAR_ADDR_OFFSET, bit_len,
                               result + 4 + workchain_len + 1);
        result[full_len] = '\0';
        PG_FREE_IF_COPY(addr, 0);
        PG_RETURN_CSTRING(result);
    }

    workchain_len = snprintf(workchain, sizeof(workchain), "%d:", tonaddr_read_i32(data + TONADDR_WORKCHAIN_OFFSET));
...
static int tonaddr_cmp_internal(TonAddr *a, TonAddr *b) {
    ...
    uint8 akind = adata[TONADDR_KIND_OFFSET];
    uint8 bkind = bdata[TONADDR_KIND_OFFSET];
    int32 aworkchain;
    int32 bworkchain;
    int abit_len;
    int bbit_len;
    ...
    switch (akind) {
        case TONADDR_KIND_NONE:
            return 0;
        case TONADDR_KIND_EXT:
            abit_len = tonaddr_read_u16(adata + TONADDR_EXT_BITS_OFFSET);
            bbit_len = tonaddr_read_u16(bdata + TONADDR_EXT_BITS_OFFSET);
        ...
        case TONADDR_KIND_STD:
            aworkchain = tonaddr_read_i32(adata + TONADDR_WORKCHAIN_OFFSET);
            bworkchain = tonaddr_read_i32(bdata + TONADDR_WORKCHAIN_OFFSET);
        ...
        case TONADDR_KIND_VAR:
            ...
            abit_len = tonaddr_read_u16(adata + TONADDR_VAR_BITS_OFFSET);
            bbit_len = tonaddr_read_u16(bdata + TONADDR_VAR_BITS_OFFSET);
            if (abit_len < bbit_len) {
                return -1;
            }
            if (abit_len > bbit_len) {
        ...
        default:
            return ton_compare_bytes((const char*) adata, VARSIZE_ANY_EXHDR(a),
                                     (const char*) bdata, VARSIZE_ANY_EXHDR(b));
    }
}
