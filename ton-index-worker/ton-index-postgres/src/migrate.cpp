...
void run_1_2_9_migrations(const std::string& connection_string, bool dry_run) {
  ...
  try {
    ...
    query += (
      ...
      "  watermark integer; "
      "begin "
      "  select last_scheduled_mc_seqno into watermark from classifier_watermark where id = 1 for update skip locked; "
      "  if not found then "
      "    return null; "
      "  end if; "
      "  cur := watermark + 1; "
...
void run_1_2_10_migrations(const std::string& connection_string, bool dry_run) {
  LOG(INFO) << "Running migrations to version 1.2.10";
  ...
  LOG(INFO) << "Migration to version 1.2.10 completed successfully.";
}
...
int main(int argc, char *argv[]) {
  ...
  if (current_version.has_value() && *current_version == latest_version && !rerun_last_migration) {
    ...
    if (migration_needed(current_version, Version{1, 2, 9})) {
      run_1_2_9_migrations(pg_connection_string, dry_run);
      current_version = Version{1, 2, 9};
    }
    if (rerun_last_migration || migration_needed(current_version, Version{1, 2, 10})) {
      run_1_2_10_migrations(pg_connection_string, dry_run);
      current_version = Version{1, 2, 10};
    }
