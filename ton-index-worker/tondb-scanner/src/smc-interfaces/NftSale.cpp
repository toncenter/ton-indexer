...
td::Result<GetGemsNftFixPriceSale::Result> GetGemsNftFixPriceSale::detect(
  ...
  TRY_RESULT(stack, execute_smc_method<11>(address, code_cell, data_cell, config, "get_sale_data", {},
  ...
  if (stack[0].as_int()->to_long() != 0x46495850) {
    return td::Status::Error("get_sale_data: invalid magic");
  }
...
td::Result<GetGemsNftAuction::Result> GetGemsNftAuction::detect(
    const block::StdAddress& address, const td::Ref<vm::Cell>& code_cell,
    const td::Ref<vm::Cell>& data_cell, const std::shared_ptr<block::ConfigInfo>& config) {
  if (code_cell.is_null() || data_cell.is_null()) {
    return td::Status::Error("Code or data null");
  ...
  auto stack_v4_r = execute_smc_method<26>(address, code_cell, data_cell, config, "get_auction_data_v4", {},
            {vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_slice,
            vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_slice,
  ...
  if (stack_v4_r.is_ok()) {
    ...
    if (last_member_address_cs->size() == 2 && last_member_address_cs->prefetch_ulong(2) == 0) {
      ...
      if (last_member_address.is_error()) {
        return last_member_address.move_as_error_prefix("last member address parsing failed: ");
      }
    ...
    if (mp_fee_address.is_error()) {
      return mp_fee_address.move_as_error_prefix("marketplace fee address parsing failed: ");
    }
    ...
    if (royalty_fee_address.is_error()) {
      return royalty_fee_address.move_as_error_prefix("royalty fee address parsing failed: ");
    }
    ...
    if (jetton_wallet_cs->size() == 2 && jetton_wallet_cs->prefetch_ulong(2) == 0) {
      ...
      if (jetton_wallet.is_error()) {
        return jetton_wallet.move_as_error_prefix("jetton wallet address parsing failed: ");
      }
    ...
    if (jetton_master_cs->size() == 2 && jetton_master_cs->prefetch_ulong(2) == 0) {
      ...
      if (jetton_master.is_error()) {
        return jetton_master.move_as_error_prefix("jetton master address parsing failed: ");
      }
  ...
  auto stack_r = execute_smc_method<20>(address, code_cell, data_cell, config, "get_sale_data", {},
            {vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_slice,
            vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_slice,
            vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int,
            vm::StackEntry::Type::t_slice, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int, vm::StackEntry::Type::t_int,
