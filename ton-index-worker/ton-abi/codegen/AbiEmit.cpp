...
class Emitter {
 public:
  Emitter(const ContractABI &abi, const AbiKernel &kernel, std::string out_name,
      ...
      : abi_(abi), kernel_(kernel), out_name_(std::move(out_name)), validate_manifest_(validate_manifest) {}
 ...
 private:
  ...
  const ValidateManifest &validate_manifest_;
  std::string snake_;
  std::string ns_;
  ...
  void validate_manifest() {
    for (const auto &key : validate_manifest_) {
      auto dot = key.rfind('.');
      if (dot == std::string::npos || dot == 0 || dot + 1 == key.size()) {
        throw HardFail{"invalid validation manifest entry '" + key + "' (expected StructName.field)"};
      }

      std::string struct_name = key.substr(0, dot);
      std::string field_name = key.substr(dot + 1);
      auto rs = kernel_.get_struct(struct_name);
      if (rs.is_error()) {
        throw HardFail{"validation manifest entry '" + key + "': " + rs.error().message().str()};
      }
      const ABIStruct *sd = rs.move_as_ok();
      auto it = std::find_if(sd->fields.begin(), sd->fields.end(), [&](const ABIStructField &field) {
        return field.name == field_name;
      });
      if (it == sd->fields.end()) {
        throw HardFail{"validation manifest entry '" + key + "': field not found"};
      }
      if (!it->default_value.has_value()) {
        throw HardFail{"validation manifest entry '" + key + "': field has no default_value"};
      }
      if (!default_supported(it->ty_idx)) {
        throw HardFail{"validation manifest entry '" + key + "': field default is not materializable"};
      }
    }
  }
  ...
  std::string load_expr(int ty_idx, const std::string &sv, const std::string &path) {
    DepthGuard __g(depth_, ty_idx);
    const Ty &t = ty(ty_idx);
    switch (t.kind) {
      case TyKind::IntN: {
      ...
      case TyKind::Nullable: {
        ...
        s += "    " + bind_res(nv, "__r" + std::to_string(k), load_expr(inner, sv, path));
      ...
      case TyKind::CellOf: {
        ...
        std::string it = cpp_type(inner);
        int k = ++uid_;
        std::string cell = "__cell" + std::to_string(k), cs = "__cs" + std::to_string(k);
        std::string cv = "__cv" + std::to_string(k);
        std::string s;
        s += "[&]() -> td::Result<ton_abi::gen::CellOf<" + it + ">> {\n";
        s += "    " + bind_res("__rs" + std::to_string(k), "__rr" + std::to_string(k),
                                "load_ref_cell_and_slice(" + sv + ")");
        s += "    td::Ref<vm::Cell> " + cell + " = std::move(__rs" + std::to_string(k) + ".first);\n";
        s += "    vm::CellSlice " + cs + " = *__rs" + std::to_string(k) + ".second;\n";
        s += "    " + bind_res(cv, "__rc" + std::to_string(k), load_expr(inner, cs, path));
      ...
      case TyKind::LispListOf: {
        ...
        s += "    " + bind_stat("__ls" + std::to_string(k),
                                leaf + "(" + sv + ", [&](vm::CellSlice& " + e + ") -> td::Status {\n" + "      " +
                                    bind_res(ev, "__re" + std::to_string(k), load_expr(inner, e, path)) +
      ...
      case TyKind::MapKV: {
        ...
        body += "      " + bind_res(valv, "__rv" + std::to_string(k), load_expr(m.value_ty_idx, vc, path));
  ...
  std::string store_stmt(int ty_idx, const std::string &expr, const std::string &bv, const std::string &path) {
    ...
    switch (t.kind) {
      ...
      case TyKind::Nullable: {
        ...
        s += "    " + store_stmt(inner, "(*" + expr + ")", bv, path) + "\n";
        s += "  } else {\n";
      ...
      case TyKind::CellOf: {
        ...
        s += "    " + store_stmt(inner, "(*(" + expr + ".ref))", icb, path) + "\n";
      ...
      case TyKind::LispListOf: {
        ...
        lam += leaf + "(" + bv + ", " + expr + ".size(), [&](vm::CellBuilder& " + e + ", std::size_t " + i +
               ") -> td::Status {\n";
        lam += "    " + store_stmt(inner, "(" + expr + "[" + i + "])", e, path) + "\n";
      ...
      case TyKind::ShapedTuple: {
        ...
        for (std::size_t i = 0; i < items.size(); ++i) {
          if (i) s += "\n  ";
          s += store_stmt(items[i], "std::get<" + std::to_string(i) + ">(" + expr + ")", bv, path);
        }
        return s;
      }
      case TyKind::MapKV: {
        ...
        lam += "    " + key_store + "\n";
        lam += "    " + store_stmt(m.value_ty_idx, ent + ".second", vo, path) + "\n";
        lam += "    return td::Status::OK();\n  })";
  ...
  std::string to_abi_expr(int ty_idx, const std::string &expr, std::optional<int> u_label) {
    ...
    switch (t.kind) {
      ...
      case TyKind::CellOf: {
        ...
        s += "    return AbiValue::make_cell_of(std::move(" + iv + "), " + expr + ".cell);\n";
        s += "  }()";
        return s;
      }
      case TyKind::ArrayOf:
  ...
  void emit_struct(int ti) {
    ...
    if (any_omitted) {
      ...
      for (const auto &fi : finfo) {
        std::string mn = member_name(fi.rf->name());
        std::string manifest_key = t.orig_name + "." + fi.rf->name();
        std::string le = load_expr(fi.rf->ty_idx, "cs", manifest_key);
        c += "  TRY_RESULT_PREFIX_ASSIGN(r." + mn + ", (" + le + "), " +
             cpp_str_lit(manifest_key + ": ") + ");\n";
        bool validate_default = validate_manifest_.count(manifest_key) != 0;
        if (validate_default &&
            (!fi.rf->orig->default_value.has_value() || !default_supported(fi.rf->ty_idx))) {
          throw HardFail{"validation manifest entry '" + manifest_key + "': resolved field default is not materializable"};
        }
        if (validate_default) {
          std::string expected = const_expr(*fi.rf->orig->default_value, fi.rf->ty_idx);
          int k = ++uid_;
          std::string expected_value = "__expected_default" + std::to_string(k);
          std::string got_value = "__got_value" + std::to_string(k);
          // RefInt256's member operator!= compares backing pointers, so compare bigint
          // values explicitly.
          std::string mismatch = cpp_type(fi.rf->ty_idx) == "td::RefInt256"
                                     ? "td::cmp(r." + mn + ", " + expected + ") != 0"
                                     : "r." + mn + " != " + expected;
          c += "  if (" + mismatch + ") {\n";
          c += "    TRY_RESULT(" + expected_value + ", (" +
               to_abi_expr(fi.rf->ty_idx, expected, fi.rf->u_label_ty_idx) + "));\n";
          c += "    TRY_RESULT(" + got_value + ", (" +
               to_abi_expr(fi.rf->ty_idx, "r." + mn, fi.rf->u_label_ty_idx) + "));\n";
          c += "    return td::Status::Error(PSLICE() << " + cpp_str_lit(manifest_key + ": expected ") + " << " +
               expected_value + ".to_json() << " + cpp_str_lit(", got ") + " << " + got_value +
               ".to_json());\n";
          c += "  }\n";
        }
      }
  ...
  void emit_alias(int ti) {
    ...
    if (custom_unpack) {
      ...
      c += "  return " + load_expr(tgt_ty, "cs", t.orig_name) + ";\n";
    }
    c += "}\n\n";
    c += "td::Status " + N + "_store(vm::CellBuilder& cb, const " + N + "& self) {\n";
    if (custom_pack) {
      ...
      c += "  " + store_stmt(tgt_ty, "self", "cb", t.orig_name) + "\n";
      c += "  return td::Status::OK();\n";
    }
    c += "}\n\n";
    c += "td::Result<AbiValue> " + N + "_to_abi_value(const " + N + "& self) {\n";
  ...
  void emit_enum(int ti) {
    ...
    h += "const ton_abi::gen::EnumNameTable& " + N + "_name_table();\n";
    h += "std::optional<std::string> " + N + "_name_of(const td::RefInt256& value);";
    t.h_decl = h;

    std::string c;
    // Metadata only: from_slice/store below deliberately never consult this
    // table, so non-member scalar values retain their unchecked round-trip.
    c += "static const ton_abi::gen::EnumNameTable " + N + "_name_table_data = {\n";
    for (const auto &member : ed->members) {
      c += "  {" + cpp_str_lit(member.name) + ", td::dec_string_to_int256(std::string(" +
           cpp_str_lit(member.value->to_dec_string()) + "))},\n";
    }
    c += "};\n\n";
    c += "const ton_abi::gen::EnumNameTable& " + N + "_name_table() {\n";
    c += "  return " + N + "_name_table_data;\n";
    c += "}\n\n";
    c += "std::optional<std::string> " + N + "_name_of(const td::RefInt256& value) {\n";
    c += "  return ton_abi::gen::enum_name_lookup(" + N + "_name_table_data, value);\n";
    c += "}\n\n";
    ...
    if (custom_unpack) {
      ...
      c += "  return " + (enc_int ? enc_load : load_expr(enc, "cs", t.orig_name)) + ";\n";
    }
    c += "}\n\n";
    ...
    if (custom_pack) {
      ...
      c += "  " + (enc_int ? "TRY_STATUS(" + enc_store + ");" : store_stmt(enc, "self", "cb", t.orig_name)) +
           "\n";
      c += "  return td::Status::OK();\n";
    }
    c += "}\n\n";
  ...
  void emit_union_bodies(int local_id) {
    ...
    if (ru.has_void && ru.variants.size() == 2) {
      ...
      c += "  { TRY_RESULT(__v0, (" + load_expr(t0.variant_ty_idx, "cs", N) +
      ...
      for (std::size_t idx : ru.dispatch_order) {
        ...
        c += "    TRY_RESULT(__v, (" + load_expr(v.variant_ty_idx, "cs", N) + "));\n";
...
td::Result<GeneratedFiles> emit_abi(const ContractABI &abi, const AbiKernel &kernel, const std::string &out_name,
                                    const ValidateManifest &validate_manifest) {
  try {
    Emitter em(abi, kernel, out_name, validate_manifest);
    return em.run();
