...
// (verified across all fixtures: the mangled monomorph name "ResetTo<int64>"
// lives ONLY in struct_instantiations[].struct_name, which we match by ty_idx,
// never by name). So no <...> stripping is
// needed on SymTable lookups; doing it would diverge from the reference
// (types-kernel.ts getStruct takes the name verbatim) and could mask a bug.
...
// structFieldsOf (types-kernel.ts:69-95), cell side
...
// aliasTargetOf (types-kernel.ts:103-120)
...
// createLabelsForUnion (types-kernel.ts:235-277)
...
void AbiKernel::build_union_dispatch(int union_ty_idx) {
  ...
  // dispatch -- it's matched by an empty slice at runtime (see dynamicUnpack
  // union, dynamic-serialization.ts:378-411).
...
// Construction and eager resolution
