...
def load_test_cases(rood_dir) -> Dict[str, Dict]:
    """Load test cases from YAML files."""
    test_cases = {}
    yaml_files = Path(rood_dir).joinpath(Path("tests/test_cases")).glob("*.yaml")

    for yaml_file in yaml_files:
        with open(yaml_file, 'r') as f:
            data = yaml.safe_load(f)
            test_cases.update(data.get("cases", {}))
    print(test_cases)
    return test_cases
...
def pytest_generate_tests(metafunc):
    if "case_name" in metafunc.fixturenames and "case_data" in metafunc.fixturenames:
        test_cases = load_test_cases(metafunc.config.rootdir)
        metafunc.parametrize("case_name,case_data", list(test_cases.items()),
...
def check_value(actual_value: Any, expected_value: Any, path: str) -> None:
    ...
    if isinstance(expected_value, str) and expected_value.startswith("{{") and expected_value.endswith("}}"):
        ...
        assert set(actual_value) == set(expected_value), f"Lists don't match at '{path}':\nExpected: {expected_value}\nActual: {actual_value}"
...
@pytest.mark.asyncio
async def test_simple_actions(pytestconfig, case_name, case_data: Dict, traces_dir):
    """Generic test that runs against all test cases defined in YAML files."""
    ...
    if "expected-blocks" in case_data:
        for expected_block in case_data["expected-blocks"]:
            ...
            assert len(matching_blocks) == 1, f"Expected 1 block of type {block_type} but found {len(matching_blocks)} for case {case_name}"
    ...
    if "expected-actions" in case_data:
        ...
        for expected_action in case_data["expected-actions"]:
            ...
            assert len(matching_actions) == 1, f"Expected 1 action of type {action_type} but found {len(matching_actions)} for case {case_name}"
