...
from indexer.core.tasks import get_last_mc_block, fetch_blocks, insert_blocks, index_interfaces
...
class BackwardScheduler(IndexScheduler):
    ...
    async def schedule_seqnos(self):
        logger.info(f"Backward scheduler started. From {settings.bottom_seqno} to {settings.start_seqno}.")
