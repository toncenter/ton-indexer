...
Index("transactions_index_2", Transaction.account, postgresql_using='hash', postgresql_concurrently=True)
Index("transactions_index_3", Transaction.hash, postgresql_using='hash', postgresql_concurrently=True)
Index("transactions_index_4", Transaction.lt, postgresql_using='btree', postgresql_concurrently=True)
# Index("transactions_index_5", Transaction.account_state_hash_before, postgresql_using='hash', postgresql_concurrently=True)
# Index("transactions_index_6", Transaction.account_state_hash_after, postgresql_using='hash', postgresql_concurrently=True)

Index('account_states_index_1', AccountState.hash, postgresql_using='hash', postgresql_concurrently=True)
# Index('account_states_index_2', AccountState.code_hash, postgresql_using='hash', postgresql_concurrently=True)

Index("messages_index_1", Message.hash, postgresql_using='hash', postgresql_concurrently=True)
Index("messages_index_2", Message.source, postgresql_using='hash', postgresql_concurrently=True)
Index("messages_index_3", Message.destination, postgresql_using='hash', postgresql_concurrently=True)
...
# Index("messages_index_6", Message.body_hash, postgresql_using='hash', postgresql_concurrently=True)
# Index("messages_index_7", Message.init_state_hash, postgresql_using='hash', postgresql_concurrently=True)

Index("transaction_messages_index_1", TransactionMessage.transaction_hash, postgresql_using='hash', postgresql_concurrently=True)
Index("transaction_messages_index_2", TransactionMessage.message_hash, postgresql_using='hash', postgresql_concurrently=True)

Index("message_contents_index_1", MessageContent.hash, postgresql_using='hash', postgresql_concurrently=True)

Index("jetton_wallets_index_1", JettonWallet.address, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_wallets_index_2", JettonWallet.owner, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_wallets_index_3", JettonWallet.jetton, postgresql_using='hash', postgresql_concurrently=True)
# Index("jetton_wallets_index_4", JettonWallet.code_hash, postgresql_using='hash', postgresql_concurrently=True)

Index("jetton_masters_index_1", JettonMaster.address, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_masters_index_2", JettonMaster.admin_address, postgresql_using='hash', postgresql_concurrently=True)
# Index("jetton_masters_index_3", JettonMaster.code_hash, postgresql_using='hash', postgresql_concurrently=True)

# Index("jetton_transfers_index_1", JettonTransfer.transaction_hash, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_transfers_index_2", JettonTransfer.source, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_transfers_index_3", JettonTransfer.destination, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_transfers_index_4", JettonTransfer.jetton_wallet_address, postgresql_using='hash', postgresql_concurrently=True)
# Index("jetton_transfers_index_5", JettonTransfer.response_destination, postgresql_using='hash', postgresql_concurrently=True)

# Index("jetton_burns_index_1", JettonBurn.transaction_hash, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_burns_index_2", JettonBurn.owner, postgresql_using='hash', postgresql_concurrently=True)
Index("jetton_burns_index_3", JettonBurn.jetton_wallet_address, postgresql_using='hash', postgresql_concurrently=True)

Index("nft_collections_index_1", NFTCollection.address, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_collections_index_2", NFTCollection.owner_address, postgresql_using='hash', postgresql_concurrently=True)
# Index("nft_collections_index_3", NFTCollection.code_hash, postgresql_using='hash', postgresql_concurrently=True)

Index("nft_items_index_1", NFTItem.address, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_items_index_2", NFTItem.collection_address, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_items_index_3", NFTItem.owner_address, postgresql_using='hash', postgresql_concurrently=True)

# Index("nft_transfers_index_1", NFTTransfer.transaction_hash, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_transfers_index_2", NFTTransfer.nft_item_address, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_transfers_index_3", NFTTransfer.old_owner, postgresql_using='hash', postgresql_concurrently=True)
Index("nft_transfers_index_4", NFTTransfer.new_owner, postgresql_using='hash', postgresql_concurrently=True)
