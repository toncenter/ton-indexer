...
from indexer.core.crud import (
    insert_by_seqno_core,
    get_existing_seqnos_from_list,
    get_existing_code_hashes,
    insert_code_hash_interfaces_core
)
...
class InterfaceDetector:
    ...
    async def is_nft_royalty(self, smc_id):
        ...
        try:
            ...
            read_stack_slice(r['stack'][2])  # FIXME: method not exists
        except:
            return []
        return ["nft_royalty"]
    ...
    async def is_auction(self, smc_id):
        ...
        try:
            assert len(r['stack']) == 3
            read_stack_slice(r['stack'][0])  # FIXME: method not exists
            read_stack_num(r['stack'][1])
...
class IndexWorker():
    def __init__(self, loop, ls_index, use_ext_method=False, cdll_path=None):
        with open(settings.liteserver_config, 'r') as f:
            tonlib_config = json.loads(f.read())
        
        keystore = f'/tmp/ton_keystore_{ls_index}'
