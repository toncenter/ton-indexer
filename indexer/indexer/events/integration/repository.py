...
class MongoRepository:
    ...
    def has_event(self, trace_id: str) -> bool:
        collection = self.db["events"]
        _id = bson.ObjectId(trace_id.encode())
        return collection.count_documents({"_id": _id}) > 0

    def save_events(self, events: [tuple[Trace, Block]]):
        collection = self.db["events"]
        documents = [self._get_document(e, b) for e, b in events]
        collection.insert_many(documents)

    def _get_document(self, trace: Trace, block: Block) -> dict:
        actions = []
        ...
        transaction_hashes = [base64.b64decode(m.hash) for m in trace.transactions]
        lt = min([m.lt for m in trace.transactions])
        collection = self.db["events"]
        return {
            "_id": bson.ObjectId(trace.trace_id.encode()),
            "hashes": transaction_hashes,
            "timestamp": min([m.now for m in trace.transactions]),
            "lt": lt,
    ...
    def save_actions(self, trace: Trace, block: Block):
        collection = self.db["events"]
        collection.insert_one(self._get_document(trace, block))
