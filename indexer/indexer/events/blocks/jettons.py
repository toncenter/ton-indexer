...
from indexer.events.blocks.messages.jettons import JettonMint
...
class JettonTransferBlockMatcher(BlockMatcher):
    ...
    def test_self(self, block: Block):
        return (
            ...
            and block.opcode == JettonTransfer.opcode
        )

    async def build_block(
        self, block: Block | CallContractBlock, other_blocks: list[Block]
    ) -> list[Block]:
        ...
        has_internal_transfer = (
            find_call_contract(other_blocks, JettonInternalTransfer.opcode) is not None
        )
        data = {
            ...
            "forward_payload": (
                base64.b64encode(jetton_transfer_message.forward_payload).decode(
                    "utf-8"
                )
                if jetton_transfer_message.forward_payload is not None
                else None
            ),
            "custom_payload": (
                base64.b64encode(jetton_transfer_message.custom_payload).decode("utf-8")
                if jetton_transfer_message.custom_payload is not None
                else None
            ),
        ...
        if len(block.next_blocks) > 0:
            data["receiver_wallet"] = AccountId(
                block.next_blocks[0].event_nodes[0].message.destination
            )
        sender = await context.interface_repository.get().get_jetton_wallet(
            block.event_nodes[0].message.destination
        )
        if sender is not None:
            data["sender"] = AccountId(sender.owner)
            data["asset"] = Asset(is_ton=False, jetton_address=sender.jetton)
            new_block.value_flow.add_jetton(
                AccountId(sender.owner),
                AccountId(sender.jetton),
                -jetton_transfer_message.amount,
            )
            new_block.value_flow.add_jetton(
                receiver, AccountId(sender.jetton), jetton_transfer_message.amount
            )
        else:
            return []
...
async def _get_jetton_burn_data(
    new_block: Block, block: Block | CallContractBlock
) -> dict:
    jetton_burn_message = JettonBurn(block.get_body())
    wallet = await context.interface_repository.get().get_jetton_wallet(
        block.get_message().destination
    )
    assert wallet is not None
    new_block.value_flow.add_jetton(
        AccountId(wallet.owner), AccountId(wallet.jetton), -jetton_burn_message.amount
    )
    data = {
        "owner": AccountId(wallet.owner),
        "jetton_wallet": AccountId(block.get_message().destination),
        "amount": Amount(jetton_burn_message.amount),
        "asset": Asset(is_ton=False, jetton_address=wallet.jetton),
    }
    return data
...
class JettonBurnBlockMatcher(BlockMatcher):
    def __init__(self):
        super().__init__(
            child_matcher=ContractMatcher(
                opcode=JettonBurnNotification.opcode, optional=True, include_excess=True
            )
        )
